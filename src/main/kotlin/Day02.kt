...
fun day2Part2(): Int{
    ...
    lines.forEach { line ->
        when(line[0]){
            'A' -> {
                if (line[2] == 'X') totalScore += LOSS_VALUE + SCISSORS_VALUE
                if (line[2] == 'Y') totalScore += DRAW_VALUE + ROCK_VALUE
                if (line[2] == 'Z') totalScore += WIN_VALUE + PAPER_VALUE
            }
            'B' -> {
                if (line[2] == 'X') totalScore += LOSS_VALUE + ROCK_VALUE
                if (line[2] == 'Y') totalScore += DRAW_VALUE + PAPER_VALUE
                ...
                if (line[2] == 'X') totalScore += LOSS_VALUE + PAPER_VALUE
                if (line[2] == 'Y') totalScore += DRAW_VALUE + SCISSORS_VALUE
                if (line[2] == 'Z') totalScore += WIN_VALUE + ROCK_VALUE
            }
        }
    }
    return totalScore
